...
def format_dhcp_packet(pkt: packet.DHCPPacket) -> str:
    ...
    a_a  =  utils.visual_length(client_info) - (COL_LEN - client_info_padding)
    if a_a > 0:
        client_info = client_info[:-a_a]
...
class DHCPClient(object):
    ...
    def get_lease(
        ...
        start = default_timer()
        logging.debug(f"Sending discover packet to {server} with")
        self.send_discover(server, discover, verbose)
        # O
        tries = 0
        init_offer = self.receive_offer(tx_id, verbose)
        while not init_offer:
            init_offer = self.receive_offer(tx_id, verbose)
            logging.debug(f"Sleeping {self.retry_interval} ms then retrying discover")
            sleep(self.retry_interval / 1000)
            logging.debug(
                f"Attempt {tries} - Sending discover packet to {server} with "
            )
            if verbose > 1:
        ...
        logging.debug(f"Sending request packet to {server} with")
        self.send_request(server, request, verbose)
        # A
        tries = 0
        some_ack = self.receive_ack(tx_id, verbose)
        while not some_ack:
            some_ack = self.receive_ack(tx_id, verbose)
            logging.debug(f"Sleeping {self.retry_interval} ms then retrying request")
            sleep(self.retry_interval / 1000)
            logging.debug(
                f"Attempt {tries} - Sending request packet to {server} with"
            )
            if verbose > 1:
        ...
        lease = Lease(discover, offer, request, some_ack, lease_time, self.ack_server)
